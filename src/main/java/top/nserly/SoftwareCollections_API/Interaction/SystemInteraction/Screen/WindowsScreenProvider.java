    ...
    public ScreenInfo getScreenForWindow(long windowHandle) {
        ...
        try {
            ...
            for (ScreenInfo screen : screens) {
                if (rect.left >= screen.getX() && rect.left < screen.getX() + screen.getScreenSize().width &&
                        rect.top >= screen.getY() && rect.top < screen.getY() + screen.getScreenSize().height) {
                    return screen;
                }
            }
        } catch (Exception e) {
            System.err.println("Error getting screen for window: " + e.getMessage());
